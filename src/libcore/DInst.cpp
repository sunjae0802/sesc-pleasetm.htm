...
void DInst::traceTM() {
    ...
    if(tmAbortCompleteOp()) {
        ...
        bool causedByFallback = ThreadContext::isFallbackMutexAddr(abortByAddr);
        ...
        if(abortType == TM_ATYPE_DEFAULT) {
            ...
            ThreadContext::getTracefile()<<pid<<" A"
            ...
            ThreadContext::getTracefile()<<pid<<" Z"
                            <<" 0x"<<std::hex<<abortIAddr<<std::dec
                            <<" 254"
                            <<" "<<aborter
                            <<" "<< context->getNRetiredInsts()
        ...
        if(getTMBeginSubtype() == TM_BEGIN_REGULAR) {
...
void DInst::traceFunction(const FuncBoundaryData& funcData) {
    Pid_t pid = context->getPid();
    char eventType = '?';
    ...
    if(eventType != '?') {
        std::ofstream& out = ThreadContext::getTracefile();
        out << pid << ' ' << eventType;
        if(funcData.isCall) {
