...
class RealLinuxSys : public LinuxSys, public ArchDefs<mode> {
    ...
    public:
        ...
        Trobust_list_head(ThreadContext *context, VAddr addr)
            ...
            list_op_pending(fixEndian(context->readMemRaw<decltype(list_op_pending)>(addr+This::Offs_list_op_pending)))
        {
        }
    };
...
#if (defined SUPPORT_MIPS32)
template<> int RealLinuxSys<ExecModeMips32>::stackGrowthSign(void) {
    return -1;
}
#endif
#if (defined SUPPORT_MIPSEL32)
template<> int RealLinuxSys<ExecModeMipsel32>::stackGrowthSign(void) {
    return -1;
}
#endif
#if (defined SUPPORT_MIPS64)
